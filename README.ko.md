...
## 이번 업데이트 — 1.26.0925 build 1127

- 현상 설정에 기본으로 꺼진 노출 범위 확장을 추가했습니다. 켜면 인화／보기 노출을 ±12 EV로 조정할 수 있으며 미리보기, 내보내기, 레시피 저장에 적용됩니다. 꺼도 기존 범위 밖 값은 유지합니다.
- 번들 이름과 파일 이름의 불일치로 Dock／Finder에 FilmYourPhoto가 표시되는 문제를 수정했습니다. 영문 이름은 FilmDevelop이며 중국어, 일본어, 한국어 이름은 유지됩니다.
- 정보의 버전 아래에 GitHub 링크를 추가했습니다. 기본 브라우저로 저장소를 엽니다.

- 기본값 복원 시 원본으로 돌아가 사진 조정값과 편집 표시를 지우고 복원 상태를 저장합니다.
- 기본으로 켜지는 하이라이트 보호 설정을 추가했습니다. 끄면 양의 인화 노출 보정에서 하이라이트 압축을 해제하며 음의 노출에서 그림자 보호는 유지합니다. 미리보기와 내보내기에 동일하게 적용됩니다.
- 사진 전환 시 올바른 비율의 썸네일을 유지하고 디코딩된 미리보기로 0.18초 동안 부드럽게 전환합니다. 동작 줄이기 설정을 지원합니다.
- 설정을 왼쪽의 일반, 현상, MCP, 정보로 분류했습니다. 버전과 업데이트 확인은 정보로 이동하고 중복 제목을 제거했습니다.
- 썸네일 우클릭 메뉴에 파일 삭제를 추가했습니다. 확인 후 클릭한 사진을 휴지통으로 이동하며 다른 편집 중인 사진은 삭제하지 않습니다.

- 필름 호버 미리보기의 상태 표시줄에 HTML 코드가 나타나는 문제를 수정했습니다. 일반 텍스트로 통일하여 사용자 지정 이름의 `<`, `>`, `&`도 올바르게 표시합니다.
- 왼쪽 필름 목록을 드래그하여 순서를 바꾸고 다음 실행에도 유지할 수 있습니다. 사이드바를 접은 상태에서도 사용할 수 있습니다. 원본은 맨 위에 고정되며 사용자 지정 필름과 내장 필름은 각 그룹 안에서 정렬됩니다.
- 편집하지 않은 새 사진은 원본으로 시작합니다. 저장된 편집이 있는 사진은 해당 필름과 조정값을 복원합니다.
- 사진 전환과 미리보기 중 WebKit 캐시가 계속 증가하는 문제를 수정했습니다. 이미지 데이터를 매번 새로운 JavaScript 코드에 삽입하지 않고 고정 스크립트의 인수로 전달합니다.
- 축소 미리보기를 독립적인 FP32 이미지로 생성하여 작은 미리보기 캐시가 전체 원본 이미지를 함께 보관하지 않도록 했습니다.
- 통합된 필름을 기반으로 만든 사용자 지정 레시피가 컬렉션에서 숨겨지는 문제를 수정했습니다.
- 영문 앱 이름을 FilmDevelop으로 통일했습니다. 번체 중국어, 일본어, 한국어 이름은 그대로 유지됩니다.
...
처음에는 약 217 MB의 LaMa 모델을 다운로드합니다. 취소 가능한 대화 상자에 진행률과 전송량을 표시하고 모델 준비를 이어서 수행합니다. 이후 Apple Silicon의 Core ML로 오프라인 처리하며 사진을 업로드하지 않습니다. 넓은 영역은 흐리거나 부자연스러운 부분이 없는지 확인하세요.
