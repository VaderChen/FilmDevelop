...
## 이번 업데이트 — 1.26.0925 build 0134

- 처리 단계마다 임시 리소스를 해제하고 FP32 버퍼를 재사용하여 원본 해상도 내보내기의 최대 메모리 사용량을 줄였습니다.
- RAW 즉시 디코딩으로 지연 디코딩에 따른 부분적인 색상 오류를 방지합니다.
- EV를 높이면 밝은 영역을 부드럽게 압축하고 낮추면 어두운 영역을 보호합니다. 미리보기와 내보내기에 함께 적용됩니다.
- 현상 애니메이션은 최대 2048 px 캐시와 실제 처리 진행률을 사용하며, 일반적으로 처리 완료 후 약 1.2초 안에 종료됩니다.
- 사진 선택 시 썸네일 목록의 현재 위치를 유지하며 자동으로 중앙에 맞추지 않습니다. 수동 스크롤은 계속 사용할 수 있습니다.
