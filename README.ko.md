...
## 이번 업데이트 — 1.26.0924 build 2244

- 사용자 필름 카드에 내보내기를 추가하여 설정을 JSON 파일로 저장할 수 있습니다. 필름 라이브러리 제목 오른쪽에 가져오기를 추가했습니다.
- ID가 같으면 기존 설정을 덮어쓰고, ID가 다르면 새로 추가합니다. 이름만 같고 ID가 다르면 이름에 번호를 붙입니다. 가져오기는 현재 사진을 자동으로 변경하지 않습니다.
- 썸네일 파일 이름 오른쪽에 `#0001` 형식의 번호가 표시됩니다. 폴더 제목 아래에는 현재 사진 순서와 전체 장수가 표시됩니다.
- 썸네일 목록의 가로 스크롤 막대가 계속 표시되어 쉽게 이동할 수 있습니다. 세로 스크롤 막대는 조작을 멈추면 숨겨집니다.
- 사진 폴더를 선택하면 첫 번째 사진이 자동으로 열립니다. 앱을 다시 시작하면 마지막으로 편집한 사진을 복원합니다.
- 사용 중인 사용자 필름을 저장할 때 기존 이름을 자동으로 입력합니다. 같은 이름이면 해당 필름을 업데이트하고 새 이름이면 별도로 저장합니다. 다른 동명 필름은 덮어쓰지 않습니다.
