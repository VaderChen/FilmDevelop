...
## 이번 업데이트 — 1.26.0924 build 1826

- 카메라 시뮬레이션에 GR III 네거티브 / 고대비 흑백, GR IV Cinema Yellow / Green을 추가했습니다. 기본 강도는 모두 50입니다. 5개 장면 비교에서 Velvia 50과 유사한 GR III 포지티브는 제외했습니다. 제조사 LUT가 아닌 독립적으로 설계한 근사 효과입니다.
- 필름을 18종으로 정리했습니다: Portra 160 / 800 → 400, VISION 250D → 50D, 200T → 500T, FP4 → HP5. 즐겨찾기와 순서는 대표 항목으로 연결하고, 저장된 사진과 사용자 필름의 기존 설정은 유지합니다.
- 중립 스캔에서 필름별 인화 대비, 최대 농도와 잔류 은 특성이 누락되던 문제를 수정했습니다. FP32 정밀도를 유지합니다.
- 필름 라이브러리를 간결한 5열 카드로 바꾸고 분류 구분선과 이름순 정렬을 적용했습니다. 적용 버튼과 현재 필름 요약을 제거하고 작은 이름 옆에 AI 설명을 배치했습니다. 선택한 항목은 작업 공간에서 적용하세요.
- 번체 중국어 정렬로 라이브러리가 열리지 않는 문제, GR 호버 미리보기 거부, 스크롤 위치 복원 시 막대가 다시 나타나는 문제를 수정했습니다. 조작 중단 약 1초 후 스크롤 막대가 숨겨집니다.
