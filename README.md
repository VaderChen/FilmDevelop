...
- 修正 Hover 底片時狀態列顯示 HTML 語法；統一使用純文字，自訂名稱中的 `<`、`>`、`&` 也能正確顯示。
- 左側底片列表可拖曳排序，並永久記住順序；原片固定置頂，自訂與內建底片各自排序，收合時也能操作。
- 尚未編輯的新照片預設使用原片；已有編輯紀錄的照片仍還原各自的底片與參數。
- 修正反覆切圖與預覽時，圖片資料嵌入 JavaScript 原始碼導致 WebKit 快取逐次累積；改為固定程式碼與獨立參數傳遞。
- 縮小預覽立即產生獨立的 FP32 影像，避免小圖快取連帶保留完整原圖。
- 修正以已整併底片為基礎的自訂配方被收藏列表隱藏。
- 英文 App 名稱統一為 FilmDevelop；繁體中文、日文及韓文名稱維持不變。
