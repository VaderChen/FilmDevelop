...
## 本版更新 — 1.26.0925 build 0134

- 原生處理逐階段釋放資源並重用 FP32 工作緩衝區，降低完整解析度匯出的記憶體峰值。
- 修正 RAW 延後解碼可能造成的匯出局部異色。
- 提高 EV 時柔和壓縮高光，降低 EV 時保護暗部；預覽與匯出共用處理。
- 顯影動畫沿用最多 2048 px 的預覽快取，依實際處理進度顯現，一般完成後約 1.2 秒收尾。
- 選取照片時保留縮圖列的水平位置，不再自動置中；仍可手動捲動。
