...
enum PhotoStyleAIRequest {
    ...
    - film_effects.color_model: exactly "spectral": LHTSS radiance reconstruction, multi-band dye transmission and a separate print stage. The analytic three-band curve has been replaced. This switch only applies to film-stock styles. print_exposure: -4...4 EV (neutral 0), output exposure compensation: positive values brighten and negative values darken ALL film stocks, including color negatives, B&W negatives and reversal film. The renderer handles the opposite physical paper-exposure sign internally; never invert the requested value. print_contrast: 0...100 (neutral 50), print contrast for negative film or viewing-density contrast for reversal film. The same positive-bright exposure convention applies to ordinary styles. These two controls also apply to ordinary styles through a neutral digital print stage after the style signature; neutral values preserve the existing appearance.
    ...
    static let grammar: String = {
        ...
        return [
            ...
            "film ::= " + object([
                ...
                ("scanner_profile", "scanner-profile"), ("scanner_illuminant", "illuminant"),
            ...
            #"scanner-profile ::= ("\"off\"" | "\"neutral\"" | "\"warmCool\"") ws"#,
